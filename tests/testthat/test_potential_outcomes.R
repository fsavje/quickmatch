library(quickmatch)
context("potential_outcomes()")

source("../replica/potential_outcomes.R", local = TRUE)

sound_outcomes <- 1:10 / 2
unsound_outcomes <- factor(1:10)
sound_matching <- qm_matching(c(rep(0, 5), rep(1, 5)))
unsound_matching <- factor(1:10)
sound_treatments <- factor(rep(1:2, 5))
unsound_treatments <- factor(rep(1:2, 4))
